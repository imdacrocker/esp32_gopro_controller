 ...
 *
 * The device is an isolated SoftAP with no upstream internet. We hand out our
 * own IP (CONFIG_WIFI_AP_IP_ADDR) as the DHCP DNS server (see wifi_manager.c),
 * then this responder resolves only the friendly local name
 * (CONFIG_WIFI_LOCAL_DOMAIN) to that IP and answers NXDOMAIN for every other
 * name.
 *
 ...
 * about reaching outside networks. By telling the truth (only the local domain
 * exists here), the phone sees an AP with no internet and cleanly uses cellular
 * for everything else, while the local domain still loads the web UI.
 *
