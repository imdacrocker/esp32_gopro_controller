...
| [`http_server_core`](../components/http_server_core) | esp_httpd lifecycle + LittleFS mount + static assets + soft-recovery fallback + variant-agnostic `/api/*` (version, settings, ota, logs, shutdown). Returns the httpd handle so variant components can register more on top |
| [`http_server_wireless`](../apps/wireless/components/http_server_wireless) | Wireless-variant `/api/*` (pairing, cameras, RC) registered on top of http_server_core |
